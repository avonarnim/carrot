...
document
  ...
  .addEventListener("click", handleOrganize);
    ...
    for (let category in categories) {
      ...
      categories[category].forEach((item) => {
        ...
        if (titleMap[item]) {
          ...
          deleteButton.dataset.index = tabsData.findIndex(
            (tab) => tab.title === item
          ); // store the index
          deleteButton.addEventListener("click", function (e) {
            ...
            for (let category in contentObj) {
              let categoryIndex = contentObj[category].findIndex(
                (i) => i === item
              );
              if (categoryIndex !== -1) {
